...
/* ── Global Styles ── */
if (typeof document !== "undefined" && !document.getElementById("dmc-global")) {
...
const SEED = [
  ...
  { id:"DMC-005", title:"Stray dogs menace in Sarojini Nagar residential colony", desc:"Large pack of aggressive stray dogs attacking pedestrians daily. Three children bitten last week. Anti-rabies treatment required for affected residents.", dept:"Public Health", zone:"South Delhi", priority:"High", status:"Submitted", citizenId:"CIT-005", citizenName:"Renu Gupta", citizenPhone:"9654321098", citizenEmail:"renu.gupta@hotmail.com", createdAt:"27 Mar, 09:15 AM", updatedAt:"27 Mar, 09:15 AM", aiConfidence:91, timeline:[{s:"Submitted",t:"27 Mar, 09:15 AM",by:"Renu Gupta",note:""}] },
  { id:"DMC-006", title:"Illegal construction blocking public pathway in Karol Bagh", desc:"A commercial establishment has encroached upon the public footpath outside Ajmal Khan Road. Pedestrians forced to walk on the busy road.", dept:"Building & Planning", zone:"Central Delhi", priority:"Medium", status:"Under Review", citizenId:"CIT-001", citizenName:"Vikram Mehra", citizenPhone:"9876543210", citizenEmail:"citizen@delhi.gov.in", createdAt:"24 Mar, 03:00 PM", updatedAt:"25 Mar, 10:00 AM", aiConfidence:89, timeline:[{s:"Submitted",t:"24 Mar, 03:00 PM",by:"Vikram Mehra",note:""},{s:"Under Review",t:"25 Mar, 10:00 AM",by:"Admin",note:""}] },
  { id:"DMC-007", title:"Waterlogging in Mayur Vihar Phase 2 after rain", desc:"Severe waterlogging on the main road making it impassable. Drains appear blocked by construction debris. Affecting hundreds of commuters daily.", dept:"Water Supply & Drainage", zone:"East Delhi", priority:"Medium", status:"Resolved", citizenId:"CIT-006", citizenName:"Dinesh Kohli", citizenPhone:"9711223344", citizenEmail:"dinesh.k@gmail.com", createdAt:"20 Mar, 01:00 PM", updatedAt:"22 Mar, 05:30 PM", aiConfidence:93, timeline:[{s:"Submitted",t:"20 Mar, 01:00 PM",by:"Dinesh Kohli",note:""},{s:"Under Review",t:"20 Mar, 03:00 PM",by:"Admin",note:""},{s:"Assigned",t:"21 Mar, 09:00 AM",by:"Admin",note:""},{s:"In Progress",t:"21 Mar, 02:00 PM",by:"Admin",note:"Drain cleaning in progress"},{s:"Resolved",t:"22 Mar, 05:30 PM",by:"Admin",note:"Drains cleared. Waterlogging resolved."}] },
  { id:"DMC-008", title:"Park benches and equipment broken in Lodhi Garden area", desc:"Multiple benches in the public park near housing society are broken and rusted. Playground equipment poses risk to children. Park looks neglected.", dept:"Parks & Gardens", zone:"New Delhi", priority:"Low", status:"Submitted", citizenId:"CIT-007", citizenName:"Meena Joshi", citizenPhone:"9988776655", citizenEmail:"meena.joshi@rediff.com", createdAt:"27 Mar, 07:45 AM", updatedAt:"27 Mar, 07:45 AM", aiConfidence:88, timeline:[{s:"Submitted",t:"27 Mar, 07:45 AM",by:"Meena Joshi",note:""}] },
];

/* ── In-memory store (with localStorage persistence) ── */
const STORAGE_KEY = "dmc_complaints";
const loadStore = () => {
  try {
...
let COMPLAINT_STORE = loadStore();
let STORE_LISTENERS = [];
const subscribeStore = (fn) => { STORE_LISTENERS.push(fn); return () => { STORE_LISTENERS = STORE_LISTENERS.filter(f => f !== fn); }; };
const publishStore = () => { saveStore(COMPLAINT_STORE); STORE_LISTENERS.forEach(fn => fn([...COMPLAINT_STORE])); };
const addComplaint = (c) => { COMPLAINT_STORE = [c, ...COMPLAINT_STORE]; publishStore(); };
const updateComplaint = (id, changes) => {
  COMPLAINT_STORE = COMPLAINT_STORE.map(c => c.id === id ? {
    ...c, ...changes,
    timeline: changes.status && changes.status !== c.status
      ? [...c.timeline, { s: changes.status, t: now(), by: "Admin", note: changes.adminNote || "" }]
      : c.timeline,
    updatedAt: now(),
  } : c);
  publishStore();
...
const useStore = () => {
  const [data, setData] = useState([...COMPLAINT_STORE]);
  useEffect(() => { const unsub = subscribeStore(setData); return unsub; }, []);
  return data;
};
...
/* ── API helper — calls local backend proxy ── */
const callAI = async (system, userMsg, max_tokens = 300) => {
...
/* ── Complaint Modal ── */
const ComplaintModal = ({ complaint, onClose, isAdmin }) => {
  ...
  const doSave = async () => {
    ...
    updateComplaint(complaint.id, { status: newStatus, adminNote: note });
    setSaving(false);
    onClose();
  };
  ...
  const runAI = async () => {
    ...
    try {
      const text = await callAI(
        "You are an AI assistant for Delhi Municipal Corporation's grievance system. Provide a concise 2-3 sentence action recommendation for the officer. Be specific and direct.",
...
const CitizenApp = ({ user, onLogout }) => {
  ...
  const classifyAI = useCallback(async (title, desc) => {
    ...
    try {
      const text = await callAI(
        'You classify municipal complaints for Delhi Municipal Corporation. Reply ONLY with valid JSON: {"dept":"<choose from: Sanitation & Solid Waste|Roads & Infrastructure|Water Supply & Drainage|Electricity & Streetlights|Public Health|Parks & Gardens|Building & Planning|Traffic & Transport|Other>","priority":"<Low|Medium|High|Critical>","confidence":<80-99>}',
        `Title: ${title}\nDescription: ${desc}`,
        100
      );
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.surface, display:"flex", flexDirection:"column" }}>
            ...
            {submitted ? (
              ...
              <div style={{ background:T.white, border:`1px solid ${T.border}`, borderRadius:5, padding:24, display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <div>
                  ...
                  <textarea style={{...inp,resize:"vertical",minHeight:90,lineHeight:1.65}} placeholder="Location, duration, severity, impact on residents…" value={form.desc} onChange={e=>handleTxt("desc",e.target.value)}/>
                </div>
                  ...
                  <div style={{ background:T.blueL, border:`1px solid ${T.navy}18`, borderRadius:4, padding:"10px 13px" }}>
                    ...
                    {classified && !classifying && (
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                        ...
                        <span style={{ fontSize:12 }}>Priority: <b style={{color:PRIORITY_META[classified.priority]?.c}}>{classified.priority}</b></span>
      ...
      {selected && <ComplaintModal complaint={selected} onClose={()=>setSelected(null)} isAdmin={false}/>}
    </div>
  );
};
...
const AdminApp = ({ user, onLogout }) => {
  ...
  const getAIReport = async () => {
    ...
    try {
      ...
      const text = await callAI(
        "You are an AI system analyst for Delhi Municipal Corporation grievance management. Generate exactly 3 numbered actionable executive insights in plain text, no markdown.",
        `Complaint analytics: ${summary}`,
        280
      );
      setAiReport(text || "Report unavailable.");
    } catch {
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.surface, display:"flex", flexDirection:"column" }}>
      ...
      {selected && <ComplaintModal complaint={selected} onClose={()=>{ setSelected(null); }} isAdmin={true}/>}
    </div>
  );
};
